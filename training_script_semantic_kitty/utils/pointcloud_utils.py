...
def pad_pointcloud(point_cloud, mask, target_size=10000):
    ...
    if current_size < target_size:
        padding_point_cloud = np.zeros(
            (target_size - current_size, feature_size), dtype=point_cloud.dtype
        )
        padding_mask = np.full(
            (target_size - current_size,), -1, dtype=mask.dtype
        )  # Padding label as -1
...
def canonical_transform(pointclouds, poses):
    assert len(pointclouds) == len(
        poses
    ), "Number of Pointclouds and Poses do not match"
    ...
    for i, (pc, T) in enumerate(zip(pointclouds, transforms)):
        ...
        if pc.shape[1] < 3:
            raise ValueError(
                f"Point cloud {i} has only {pc.shape[1]} columns, need at least 3"
            )
