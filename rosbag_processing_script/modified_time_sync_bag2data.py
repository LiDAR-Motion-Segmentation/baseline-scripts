...
class BagReader(Node):
    def __init__(self):
        ...
        bag_path = os.path.expanduser('/media/soumoroy/Extreme SSD/Motion-segementation-rosbags/tech_fest/2025-10-05_17-06-25/rosbag/rosbag_0.db3')
        ...
        self.topic = {
            ...
            '/camera1/camera1/aligned_depth_to_color/image_raw': (Image, self.depth_image1_pub),
            '/camera1/camera1/color/camera_info': (CameraInfo, self.camera1_info),
            '/camera2/camera2/color/image_raw': (Image, self.image2_pub),
            '/camera2/camera2/aligned_depth_to_color/image_raw': (Image, self.depth_image2_pub),
...
class BagtoData(Node):
    def __init__(self):
        ...
        self.time_sync = message_filters.ApproximateTimeSynchronizer(
            [self.image1_sub, self.image2_sub, self.equirectangular_sub,  self.lidar_sub, self.world2ego_sub],
            queue_size=50,
            slop=0.5,
        ) 
    ...
    def synchronized_callback(self, image1_msg, image2_msg, equirectangular_msg, lidar_msg, world2ego_msg):
        ...
        cv_equirectangular = self.bridge.imgmsg_to_cv2(equirectangular_msg, desired_encoding="bgr8")
        ...
        filename_equirectangular = os.path.join(self.save_equirectangular, f"{self.frame_count:06d}.png")
        ...
        quat_vec = np.array([orientation.x, orientation.y, orientation.z, orientation.w])

        rotation_matrix = ros2_numpy.geometry.transformations.quaternion_matrix(quat_vec)
        translation_matrix = ros2_numpy.geometry.transformations.translation_matrix(trans_vec)
        transform_matrix = np.dot(translation_matrix, rotation_matrix)

        world2ego_filename = os.path.join(self.save_world2ego, f"{self.frame_count:06d}.txt")
