...
def main(args):
    ...
    for pcd_path in pcd_files:
        ...
        if len(points) > 0:
            ...
            rr.log("world/lidar", rr.Points3D(points, colors=[150, 150, 150], radii=0.05))

        # B. Log Bounding Boxes
        json_path = os.path.join(args.json_dir, f"{base_name}.json")
        centers, half_sizes, quaternions, class_ids, labels = load_bounding_boxes(json_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualize .pcd, .json boxes, and images in Rerun")
    parser.add_argument("--pcd_dir", type=str, required=True, help="Directory with .pcd files")
    parser.add_argument("--json_dir", type=str, required=True, help="Directory with .json boxes")
    parser.add_argument("--img_dir", type=str, default=None, help="Root directory containing CAM_FRONT, CAM_BACK folders etc.")
