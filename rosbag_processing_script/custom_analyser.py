...
class CustomDataAnalyzer:
    ...
    def create_sync_data(
        self, cam1_imgs, cam1_intrinsics, cam2_imgs, cam2_intrinsics, lidar_files
    ) -> list:
        ...
        for timestamp in sorted(timestamp_data.keys()):
            ...
            if "lidar" in data:
                sync_entry = {
                    ...
                    "complete_frame": all(
                        k in data for k in ["lidar", "camera1_image", "camera2_image"]
                    ),
                }
...
def main():
    ...
    if sync_data:
        output_file = (
            args.output if args.output else f"{analyzer.bag_name}_sync_map.json"
        )
        ...
        if sample_entry and sample_entry.get("camera1_intrinsics"):
            ...
            if intrinsics:
                ...
                print(
                    f"   Focal length: fx={intrinsics['fx']:.1f}, fy={intrinsics['fy']:.1f}"
                )
                print(
                    f"   Principal point: cx={intrinsics['cx']:.1f}, cy={intrinsics['cy']:.1f}"
                )
