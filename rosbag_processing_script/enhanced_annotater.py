...
class FixedMultiSensorAnnotator:
    def __init__(
        self, sync_map_file, output_dir, use_sam2=True, sam2_model_size="large"
    ):
            ...
            try:
                ...
                if self.device == "cuda":
                    ...
                    try:
                        self.device = "cpu"
                        sam2_model = build_sam2(
                            config_name, checkpoint_path, device="cpu"
                        )
                        self.sam2_predictor = SAM2ImagePredictor(sam2_model)
            ...
            if boxes is not None:
                for box in boxes:
                    try:
                        ...
                        if (
                            class_id in self.moving_classes
            ...
            for i, detection in enumerate(detections):
                try:
                    ...
                    if len(masks) > 0:
                        ...
                        detection["mask_score"] = (
                            float(scores[0]) if len(scores) > 0 else 0.0
                        )

                        print(
                            f"    ✅ SAM2 mask generated for {detection['class_name']}: "
    ...
    def annotate_frame(self, frame_data):
        ...
        camera1_available = (
            frame_data.get("has_camera1", False)
            and frame_data.get("camera1_image")
            and frame_data.get("camera1_intrinsic")
        )
        ...
        if camera1_available:
            ...
            if os.path.exists(camera1_image_path) and os.path.exists(
                camera1_intrinsics_path
            ):
                try:
                    ...
                    if img1 is not None and intrinsics1 is not None:
                        ...
                        print(
                            f"     📊 Found {len(detections1)} moving object detections"
                        )
                        ...
                        for detection in detections1:
                            frustum = self.create_frustum_from_bbox(
                                detection["bbox"], intrinsics1
                            )
                            mask = self.filter_points_in_frustum(points, frustum)
                            points_labeled = np.sum(mask)
                            labels[mask] = 1
                            print(
                                f"     ✅ {detection['class_name']}: labeled {points_labeled} points"
                            )
                    else:
        ...
        camera2_available = (
            frame_data.get("has_camera2", False)
            and frame_data.get("camera2_image")
            and frame_data.get("camera2_intrinsic")
        )
        ...
        if camera2_available:
            ...
            if os.path.exists(camera2_image_path) and os.path.exists(
                camera2_intrinsics_path
            ):
                try:
                    ...
                    if img2 is not None and intrinsics2 is not None:
                        ...
                        print(
                            f"     📊 Found {len(detections2)} moving object detections"
                        )
        ...
        annotation_data = {
            ...
            "moving_ratio": (
                float(np.sum(labels == 1) / len(points)) if len(points) > 0 else 0.0
            ),
    ...
    def create_visualization(self, points, labels, frame_data, detections, timestamp):
        ...
        for camera_name in ["camera1", "camera2"]:
            self.create_camera_visualization(
                frame_data, detections, timestamp, camera_name
            )
    ...
    def create_camera_visualization(
        self, frame_data, detections, timestamp, camera_name
    ):
        ...
        try:
            ...
            camera_detections = [
                d for d in detections if d.get("camera") == camera_name
            ]
            ...
            for det in camera_detections:
                ...
                if self.use_sam2 and det.get("mask") is not None:
                    ...
                    mask_overlay = cv2.addWeighted(
                        mask_overlay, 1.0, colored_mask, 0.4, 0
                    )
            ...
            info_parts = [
                f"Frame: {timestamp}",
                f"{camera_name.upper()}",
                f"Detections: {len(camera_detections)}",
            ]
            if self.use_sam2:
                masks_count = sum(
                    1 for d in camera_detections if d.get("mask") is not None
                )
            ...
            cv2.putText(
                img, info_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 1
            )
            ...
            if camera_detections:
                ...
                cv2.rectangle(
                    img, (0, 0), (img.shape[1] - 1, img.shape[0] - 1), border_color, 5
                )
            ...
            if success:
                ...
                print(
                    f"  ❌ Failed to save {camera_name} visualization: {viz_file.name}"
                )
        ...
        try:
            for camera_name in ["camera1", "camera2"]:
                ...
                detections_with_masks = [
                    d for d in camera_detections if d.get("mask") is not None
                ]

                if detections_with_masks:
                    print(
                        f"  💾 Saving SAM2 masks for {camera_name}: {len(detections_with_masks)} masks"
                    )
                    ...
                    for i, det in enumerate(detections_with_masks):
                        if det.get("mask") is not None:
                            ...
                            mask_dir = (
                                self.output_dir / "segmentation_masks" / camera_name
                            )
                            individual_file = (
                                mask_dir
                                / f"{timestamp:019d}_{i:02d}_{class_name}_{confidence:.2f}.png"
                            )
                    ...
                    combined_file = (
                        self.output_dir
                        / "segmentation_masks"
                        / f"{timestamp:019d}_{camera_name}_combined.png"
                    )
                    cv2.imwrite(str(combined_file), combined_mask)

                    print(
                        f"    ✅ Saved individual and combined masks for {camera_name}"
                    )
    ...
    def process_all_frames(self, max_frames=None):
        ...
        for frame_data in tqdm(frames_to_process, desc="Annotating frames"):
            try:
                ...
                print(
                    f"❌ Error processing frame {frame_data.get('frame_id', 'unknown')}: {e}"
                )
                traceback.print_exc()
                continue
    ...
    def print_statistics(self):
        ...
        frames_with_detections = sum(
            1 for ann in self.annotation if ann["detections_2d"]
        )
        ...
        print(
            f"   Moving points: {total_moving:,} ({total_moving/total_points*100:.1f}%)"
        )
        print(
            f"   Static points: {total_static:,} ({total_static/total_points*100:.1f}%)"
        )
        ...
        if self.annotation:
            ...
            sam2_working = sum(
                1 for ann in self.annotation if ann.get("sam2_working", False)
            )
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", default="fixed_annotations", help="Output directory"
    )
    parser.add_argument(
        "--max_frames", "-n", type=int, help="Maximum frames to process"
    )
    parser.add_argument(
        "--disable_sam2", action="store_true", help="Disable SAM2 segmentation"
    )
    parser.add_argument(
        "--sam2_model",
