...
def main():
    parser = argparse.ArgumentParser(
        description="Extract synchronized camera and LiDAR scans from ROS2 bags"
    )
    ...
    parser.add_argument(
        "--camera-topic", required=True, help="Primary camera image topic"
    )
    parser.add_argument(
        "--camera-info-topic", required=True, help="Primary camera info topic"
    )
    parser.add_argument(
        "--camera-topic-2", default=True, help="Secondary camera image topic (optional)"
    )
    parser.add_argument(
        "--camera-info-topic-2",
        default=True,
        help="Secondary camera info topic (optional)",
    )
    parser.add_argument(
        "--camera-topic-3", default=True, help="Third camera image topic (optional)"
    )
    ...
    parser.add_argument(
        "--multi-camera", action="store_true", help="Enable dual camera processing mode"
    )

    args = parser.parse_args()

    # Validate dual camera configuration
    if args.multi_camera and (
        not args.camera_topic_2
        or not args.camera_info_topic_2
        or not args.camera_topic_3
    ):
        parser.error(
            "--multi-camera requires both --camera-topic-2 and --camera-topic-3"
        )
    ...
    if args.camera_topic in topic_id_map:
        ...
        camera_msgs = [
            (timestamp, data, topic_type_map[args.camera_topic])
            for timestamp, data in camera_rows
        ]
    ...
    if args.camera_info_topic in topic_id_map:
        ...
        caminfo_msgs = [
            (timestamp, data, topic_type_map[args.camera_info_topic])
            for timestamp, data in caminfo_rows
        ]

    # Secondary camera (conditional)
    if (
        args.multi_camera
        and args.camera_topic_2 in topic_id_map
        and args.camera_topic_3 in topic_id_map
    ):
        ...
        camera2_msgs = [
            (timestamp, data, topic_type_map[args.camera_topic_2])
            for timestamp, data in camera2_rows
        ]
        ...
        camera3_msgs = [
            (timestamp, data, topic_type_map[args.camera_topic_3])
            for timestamp, data in camera3_rows
        ]
    ...
    if args.multi_camera and args.camera_info_topic_2 in topic_id_map:
        ...
        caminfo2_msgs = [
            (timestamp, data, topic_type_map[args.camera_info_topic_2])
            for timestamp, data in caminfo2_rows
        ]
    ...
    if args.lidar_topic in topic_id_map:
        ...
        lidar_msgs = [
            (timestamp, data, topic_type_map[args.lidar_topic])
            for timestamp, data in lidar_rows
        ]

    conn.close()

    print(
        f"Found {len(camera_msgs)} primary camera messages, {len(lidar_msgs)} LiDAR messages"
    )
    if args.multi_camera:
    ...
    if args.multi_camera:
        sync_pairs_secondary = synchronize_with_lidar(
            lidar_msgs, camera2_msgs, args.sync_tol
        )
        sync_pairs_tertiary = synchronize_with_lidar(
            lidar_msgs, camera3_msgs, args.sync_tol
        )
        print(
            f"Found {len(sync_pairs_primary)} primary and {len(sync_pairs_secondary)} secondary sync pairs and {len(sync_pairs_tertiary)} tertiary pairs"
        )
    else:
    ...
    if args.multi_camera:
        print(
            f"Processing complete. Primary: {len(sync_pairs_primary)}, Secondary: {len(sync_pairs_secondary)} pairs, Tertiary: {len(sync_pairs_tertiary)} pairs"
        )
    else:
        print(
            f"Processing complete. Saved {len(sync_pairs_primary)} synchronized pairs"
        )
