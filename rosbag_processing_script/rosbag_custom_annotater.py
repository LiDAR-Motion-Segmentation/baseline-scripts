...
class CustomMultiSensorAnnotator:
                ...
                for box in boxes:
                    try:
                        ...
                        if (
                            ...
                            and confidence >= confidene_threshold
                        ):
                            ...
                            if x2 > x1 and y2 > y1:
                                ...
                                print(
                                    f"Invalid bbox coordinates: x1={x1}, y1={y1}, x2={x2}, y2={y2}"
                                )
    ...
    def create_camera_visualization(self, frame_data, detections, timestamp):
        ...
        if frame_data.get("has_camera1") and frame_data.get("camera1_image"):
            ...
            if os.path.exists(camera1_image):
                camera1_detections = [
                    d for d in detections if d.get("camera") == "camera1"
                ]
        ...
        if frame_data.get("has_camera2") and frame_data.get("camera2_image"):
            ...
            if os.path.exists(camera2_image):
                camera2_detections = [
                    d for d in detections if d.get("camera") == "camera2"
                ]
                self.process_single_camera_visualization(
                    camera_name="camera2",
    ...
    def process_single_camera_visualization(
        self, camera_name, image_file, detections, timestamp, output_dir
    ):
        try:
            ...
            for i, det in enumerate(camera_detections):
                try:
                    ...
                    print(
                        f"Drew detection {i+1}: {class_name} at [{x1}, {y1}, {x2}, {y2}] conf:{confidence:.2f}"
                    )
            ...
            if camera_detections:
                cv2.rectangle(
                    img, (0, 0), (img.shape[1] - 1, img.shape[0] - 1), (0, 255, 0), 5
                )
...
def main():
    ...
    parser.add_argument(
        "--output", "-o", default="annotations", help="Output directory"
    )
    parser.add_argument(
