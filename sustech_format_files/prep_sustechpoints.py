...
def main():
    ...
    ap.add_argument("--pcd_only", action="store_true",
                    help="Index only .pcd files (ignore .bin)")
    ap.add_argument("--force_pc_ext", choices=["keep", "pcd", "bin"], default="keep",
    ...
    ensure_dir(out_cam1); ensure_dir(out_cam2); ensure_dir(out_lidar); ensure_dir(out_cal_cam); ensure_dir(out_label)
    ...
    if not cam1: print("WARNING: no CAMERA1 PNGs found.")
    if not cam2: print("WARNING: no CAMERA2 PNGs found.")
    if not pcds: print("WARNING: no LiDAR .pcd/.bin found.")
    ...
    if args.force_pc_ext == "pcd":
        ...
        force_ext = ".bin"
    pc_new = copy_and_reindex(pcds, out_lidar, ext_preserve=(force_ext is None), force_ext=force_ext) if pcds else []

    # Lists (optional but useful for quick checks)
    if cam1_new:
        write_list(dst_scene / "CAMERA1_filenames.txt", [f"camera/CAMERA1/{n}" for n in cam1_new])
    if cam2_new:
        write_list(dst_scene / "CAMERA2_filenames.txt", [f"camera/CAMERA2/{n}" for n in cam2_new])
    if pc_new:
        # Although historically named point_cloud_filenames, we now list lidar paths
        write_list(dst_scene / "point_cloud_filenames.txt", [f"lidar/{n}" for n in pc_new])
        # Matching annotation list for convenience
        write_list(dst_scene / "annotation_filenames.txt", [f"label/{Path(n).stem}.json" for n in pc_new])
    ...
    if c1_npz and cam1_new:
        ...
        with open(out_cal_cam / "CAMERA1.json", "w") as f: json.dump(intr1, f, indent=2)
    else:
    ...
    if c2_npz and cam2_new:
        ...
        with open(out_cal_cam / "CAMERA2.json", "w") as f: json.dump(intr2, f, indent=2)
    else:
    ...
    min_len = min([x for x in [len(cam1_new), len(cam2_new), len(pc_new)] if x > 0], default=0)
    if min_len > 0 and (len(cam1_new) != len(cam2_new) or len(cam1_new) != len(pc_new)):
        print(f"NOTE: Counts mismatch; you may annotate using the common prefix of ~{min_len} frames.")
