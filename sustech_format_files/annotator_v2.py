...
def auto_annotate(
    ...
    if visualize:
        ...
        colors = [[random.randint(0, 255) for _ in range(3)] for _ in range(len(det_model.names))]
        ...
        if class_ids := result.boxes.cls.int().tolist():
            ...
            with open(box_label_path, "w", encoding="utf-8") as f_box:
                for i, box in enumerate(boxes_xywhn):
                    ...
                    f_box.write(f"{class_ids[i]} {x_center:.5f} {y_center:.5f} {width:.5f} {height:.5f}\n")
                    
            sam_results = sam_model(result.orig_img, bboxes=boxes_xyxy, verbose=False, save=False, device=device)
            segments = sam_results[0].masks.xyn
            seg_label_path = seg_dir / f"{Path(result.path).stem}.txt"
            with open(seg_label_path, "w", encoding="utf-8") as f_seg:
                for i, s in enumerate(segments):
                    if s.any():
                        segment_str = " ".join(map(lambda x: f"{x:.5f}", s.reshape(-1).tolist()))
            ...
            if visualize:
                ...
                timestep = count_files(dir="/media/soumoroy/Extreme SSD/Motion-segementation-rosbags/tech_fest/data/husky_dataset/equirectangular_auto_annotate_labels/")
                ...
                for i, seg_points_normalized in enumerate(segments):
                    ...
                    seg_points = (seg_points_normalized * np.array([W, H])).astype(np.int32)
