...
class MallTrackingRefiner:
    ...
    def load_frame(self):
        ...
        if screen_w < 100: screen_w = 1000 # Fallback during init
        if screen_h < 100: screen_h = 800
        ...
        # reset selection
        self.selected_obj_idx = -1
        self.lbl_frameconfig(text=f"{json_path.name} ({self.current_idx + 1}/{len(self.files)})")
        self.redraw()
    ...
    def redraw(self):
        ...
        for idx, obj in enumerate(self.current_data):
            ...
            # Draw Label
            label_text = f"{gid}"
            self.canvas.create_text(x1, y1-10, text=label_text, fill=fill_txt, anchor=tk.SW, font=("Arial", 12, "bold"))
    ...
    def update_control_panel(self):
        if self.selected_obj_idx >= 0:
            ...
            self.frame_edit.config(text=f"Editing Object #{self.selected_obj_idx}", fg="black")
        else:
            self.ent_id.delete(0, tk.END)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Manual Refiner Tool for Mall Tracking Data")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--cam", type=str, help="Camera ID to edit (e.g., '1' or 'cam_1')")
    group.add_argument("--input_dir", type=str, help="Full path to a specific 'cam_X' folder")
    parser.add_argument("--root", type=str, default="./output_data", help="Root output directory (default: ./output_data)")
