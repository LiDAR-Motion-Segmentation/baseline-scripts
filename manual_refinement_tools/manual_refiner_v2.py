...
class MallTrackingRefiner:
    ...
    def setup_ui(self):
        ...
        self.progress = ttk.Progressbar(self.top_bar, variable=self.progress_var, maximum=len(self.files))
        self.progress.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=10, pady=10)
        
        self.lbl_progress = tk.Label(self.top_bar, text="0 / 0", bg=c.panel_bg, fg=c.text_primary, font=f.mono)
        ...
        # Info Box
        self.info_box = tk.Label(self.sidebar, text="Ready", bg="#333", fg=c.accent, font=f.main, pady=5)
        self.info_box.pack(fill=tk.X, pady=(0, 10))

        # Selection Panel
        self.frm_sel = tk.LabelFrame(self.sidebar, text=" ACTIVE SELECTION ", bg=c.panel_bg, fg=c.highlight, font=f.header)
        self.frm_sel.pack(fill=tk.X, padx=10, pady=5)

        # ID Row
        frm_id = tk.Frame(self.frm_sel, bg=c.panel_bg)
        frm_id.pack(fill=tk.X, pady=5, padx=5)
        tk.Label(frm_id, text="Global ID:", bg=c.panel_bg, fg=c.text_primary).pack(side=tk.LEFT)
        self.ent_id = tk.Entry(frm_id, width=8, bg=c.entry_bg, fg=c.entry_fg, insertbackground="white", font=f.mono)
        self.ent_id.pack(side=tk.LEFT, padx=5)
        tk.Button(frm_id, text="SET", bg=c.accent, fg="white", relief="flat", command=self.update_id).pack(side=tk.LEFT)

        # Propagate ID
        tk.Button(self.frm_sel, text=" Propagate ID Forward (P)", bg=c.button_bg, fg=c.text_primary, relief="flat", command=self.propagate_id).pack(fill=tk.X, padx=5, pady=5)

        # Status Row
        frm_stat = tk.Frame(self.frm_sel, bg=c.panel_bg)
        frm_stat.pack(fill=tk.X, pady=5, padx=5)
        tk.Label(frm_stat, text="Status:", bg=c.panel_bg, fg=c.text_primary).pack(side=tk.LEFT)
        self.lbl_status = tk.Label(frm_stat, text="-", width=8, bg="#333", fg="white", font=f.header)
        self.lbl_status.pack(side=tk.LEFT, padx=5)
        tk.Button(frm_stat, text=" FLIP (M)", bg="#555", fg="white", relief="flat", command=lambda: self.toggle_status(False)).pack(side=tk.LEFT)

        # Propagate Status
        tk.Button(self.frm_sel, text=" Propagate Status Fwd (Shift+M)", bg=c.button_bg, fg=c.text_primary, relief="flat", command=lambda: self.toggle_status(True)).pack(fill=tk.X, padx=5, pady=5)

        # Delete
        tk.Button(self.frm_sel, text=" DELETE OBJECT (Del)", bg=c.danger, fg="white", font=f.header, relief="flat", command=self.delete_object).pack(fill=tk.X, padx=5, pady=10)

        # Navigation Panel
        tk.Label(self.sidebar, text=" NAVIGATION ", bg=c.panel_bg, fg=c.text_secondary, font=f.header).pack(pady=(20, 5))
        
        frm_nav = tk.Frame(self.sidebar, bg=c.panel_bg)
        frm_nav.pack(fill=tk.X, padx=10)
        
        tk.Button(frm_nav, text="< Prev", bg=c.button_bg, fg=c.text_primary, height=2, width=10, relief="flat", command=self.prev_frame).pack(side=tk.LEFT)
        tk.Button(frm_nav, text="Next >", bg=c.success, fg="white", height=2, width=10, relief="flat", command=self.next_frame).pack(side=tk.RIGHT)

        # Jump Buttons
        frm_jump = tk.Frame(self.sidebar, bg=c.panel_bg)
        frm_jump.pack(fill=tk.X, padx=10, pady=10)
        tk.Button(frm_jump, text="+50", bg="#444", fg="#aaa", relief="flat", command=lambda: self.jump_frame(50)).pack(side=tk.RIGHT, padx=2)
        tk.Button(frm_jump, text="+10", bg="#444", fg="#aaa", relief="flat", command=lambda: self.jump_frame(10)).pack(side=tk.RIGHT, padx=2)
        tk.Label(frm_jump, text="Jump:", bg=c.panel_bg, fg=c.text_secondary).pack(side=tk.RIGHT)
    ...
    def log(self, message, level="info"):
        ...
        self.root.after(3000, lambda: self.info_box.config(text="Ready", fg=self.cfg.colors.accent))
    ...
    def redraw(self):
        ...
        for idx, obj in enumerate(self.current_data):
            ...
            self.canvas.create_text(x1, y1-15, text=f"{gid}", fill=txt_c, anchor=tk.SW, font=self.fonts.large_label)
            ...
            if 'segmentation_polygon' in obj and obj['segmentation_polygon']:
                for poly in obj['segmentation_polygon']:
                    ...
                    self.canvas.create_polygon(scaled_poly, outline=outline, fill='', width=1, dash=(2, 2))
    ...
    def update_control_panel(self):
        ...
        if self.selected_obj_idx >= 0:
            ...
            if stat == "Moving": self.lbl_status.config(fg=c.danger)
            else: self.lbl_status.config(fg=c.accent)
            
            self.frm_sel.config(text=f" OBJECT {self.selected_obj_idx} SELECTED ", fg=c.highlight)
        else:
            self.ent_id.delete(0, tk.END)
    ...
    def propagate_id(self):
        ...
        try: new_global_id = int(self.ent_id.get())
        ...
        for i in range(self.current_idx + 1, len(self.files)):
            ...
            with open(load_p, 'r') as f: data = json.load(f)
            ...
            for obj in data:
                if obj.get('local_id') == local_id_to_track:
                    ...
                    found = True
            if found:
                with open(f_out, 'w') as f: json.dump(data, f, indent=2)
                count += 1
    ...
    def next_frame(self):
        ...
        if self.current_idx < len(self.files) - 1:
            self.current_idx += 1
            self.load_frame()
        else: messagebox.showinfo("Done", "End of sequence!")
            ...
            for obj in self.current_data:
                ...
                if 'segmentation_polygon' in obj and obj['segmentation_polygon']:
                    ...
                    for poly_part in obj['segmentation_polygon']:
                         if isinstance(poly_part, list): flat_poly.extend(poly_part)
                         else: flat_poly.append(poly_part)
                    normalized_points = []
...
if __name__ == "__main__":
    ...
    out_path = Path(args.out_dir) if args.out_dir else json_path.parent / f"{json_path.name}_corrected"
    ...
    if not cfg_path.exists():
        ...
        print(f"Please ensure 'conf/ui_theme.yaml' exists or pass a path via --config\n")
        sys.exit(1)
