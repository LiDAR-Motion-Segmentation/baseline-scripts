...
def parse_jrdb_cameras(camera_data):
    ...
    print(
        f"Found stitching parameters: radius={stitching_params.get('radius', 'N/A')}, "
    ...
    for sensor_id, sensor_params in camera_sensors.items():
        ...
        try:
            ...
            print(
                f"Parsed {sensor_id}: {width}x{height}, "
...
def generate_kitti_calibration_from_jrdb(
    cameras, stitching_params, stereo_baseline=None, use_cylindrical=False
):
    """
    ...
    if use_cylindrical:
        ...
        f_cyl, y0_cyl, W_cyl, H_cyl = convert_to_cylindrical_projection(
            cameras, stitching_params
        )
    ...
    return {"P0": P0, "P1": P1, "P2": P2, "P3": P3, "Tr": Tr}
...
def save_kitti_calibration(calib_matrices, output_file):
    ...
    with open(output_file, "w") as f:
        ...
        f.write(
            f"P0: {' '.join([f'{val:.6e}' for val in calib_matrices['P0'].flatten()])}\n"
        )
        f.write(
            f"P1: {' '.join([f'{val:.6e}' for val in calib_matrices['P1'].flatten()])}\n"
        )
        f.write(
            f"P2: {' '.join([f'{val:.6e}' for val in calib_matrices['P2'].flatten()])}\n"
        )
        f.write(
            f"P3: {' '.join([f'{val:.6e}' for val in calib_matrices['P3'].flatten()])}\n"
        )

        # Write transformation matrix (flatten 3x4 matrix to 12 values)
        f.write(
            f"Tr: {' '.join([f'{val:.6e}' for val in calib_matrices['Tr'].flatten()])}\n"
        )
...
def analyze_camera_configuration(cameras):
    ...
    print(
        f"Focal length range (fx): {min(focal_lengths_x):.1f} - {max(focal_lengths_x):.1f}"
    )
    print(
        f"Focal length range (fy): {min(focal_lengths_y):.1f} - {max(focal_lengths_y):.1f}"
    )
    ...
    print(
        f"Principal point range (cx): {min(principal_x):.1f} - {max(principal_x):.1f}"
    )
    print(
        f"Principal point range (cy): {min(principal_y):.1f} - {max(principal_y):.1f}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert JRDB multi-camera YAML to KITTI format"
    )
    parser.add_argument(
        "--camera_yaml", type=str, required=True, help="Path to JRDB camera.yaml file"
    )
    parser.add_argument(
        "--output",
        type=str,
