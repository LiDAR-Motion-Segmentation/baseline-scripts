...
def process_image_sequence(config, input_dir, output_dir, output_suffix="residual"):
    ...
    if visualize:
        visualization_folder = os.path.join(
            output_dir, f"visualization_{output_suffix}"
        )
        check_and_makedirs(residual_image_folder)
    ...
    for frame_idx in tqdm(range(len(image_paths)), desc="Processing images"):
        ...
        valid_mask = (
            (current_image >= config["min_range"])
        ...
        np.save(file_name, diff_image)
    print(
        f"Residual image generation completed! Output saved to: {residual_image_folder}"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate residual images from image sequence"
    )
    parser.add_argument("input_dir", type=str, help="Input directory containing images")
    parser.add_argument(
        "output_dir", type=str, help="Output directory for residual images"
    )
    parser.add_argument(
        "--output_suffix",
        type=str,
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug mode with matplotlib display"
    )
    parser.add_argument(
        "--num_frames",
        type=int,
