...
def estimate_fps_from_sequence_duration(
    num_frames: int, estimated_duration: Optional[float] = None
) -> float:
    """
...
def generate_timestamps_from_frame_intervals(
    frame_numbers: List[int], fps: float = 15.0
) -> np.ndarray:
    """
    ...
    timestamps = np.array(
        [(frame - min_frame) * dt for frame in sorted_frames], dtype=np.float64
    )
...
def generate_jrdb_to_kitti_times(
    input_directory: str,
    output_file: str,
    ...
    if fps is None:
        if sequence_duration is not None:
            estimated_fps = estimate_fps_from_sequence_duration(
                num_frames, sequence_duration
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate KITTI times.txt from JRDB dataset"
    )
    parser.add_argument(
        "input_directory",
        type=str,
    ...
    parser.add_argument(
        "--verify", action="store_true", help="Verify the generated times.txt file"
    )
    parser.add_argument(
        "--preview",
        type=int,
