...
def determine_motion_state(action_labels, social_activity):
    ...
    static_activities = {"sitting": 0.7, "standing": 0.6, "lying": 0.8, "sleeping": 0.8}
...
def process_single_frame(json_data, pcd_filename, bin_folder, output_folder):
    ...
    if "labels" in json_data and pcd_filename in json_data["labels"]:
        ...
        print(
            f"  {pcd_filename}: {num_points} points total, {moving_points_total} moving, {num_points - moving_points_total} static"
        )
    else:
...
def convert_jrdb_sequence_json_to_mos(json_file, bin_folder, output_folder):
    ...
    print(
        f"  Successfully converted: {successful_conversions}/{len(pcd_filenames)} frames"
    )
...
def verify_label_file(label_file, bin_file):
    ...
    try:
        ...
        if actual_size != expected_size:
            print(
                f"Error: Label file size mismatch. Expected: {expected_size}, Got: {actual_size}"
            )
            return False
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert single JRDB sequence JSON to multiple KITTI MOS label files"
    )
    parser.add_argument(
        "--json_file",
        type=str,
    ...
    parser.add_argument(
        "--verify", action="store_true", help="Verify the first converted label file"
    )
    ...
    # Convert files
    convert_jrdb_sequence_json_to_mos(
        args.json_file, args.bin_folder, args.output_folder
    )
