...
def convert_pcd_to_bin_open3d(pcd_file, bin_file):
    ...
    try:
        ...
        print(
            f"Warning: Intensity values set to zero for {pcd_file} (open3d limitation)"
        )
        return True
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert PCD files to KITTI Velodyne BIN format"
    )
    parser.add_argument(
        "--pcd_path",
        type=str,
    ...
    parser.add_argument(
        "--file_prefix", type=str, default="", help="Prefix for output BIN filenames"
    )
    parser.add_argument(
        "--verify", action="store_true", help="Verify the first converted BIN file"
    )
