...
python multicam_annotation.py \
    --camera camera1 "$CAM1_IMAGE_DIR" "$CAM1_INTRINSICS_DIR" "$CAM1_EXTRISICS_DIR" "$CAM1_LABEL" \
    --camera camera2 "$CAM2_IMAGE_DIR" "$CAM2_INTRINSICS_DIR" "$CAM2_EXTRISICS_DIR" "$CAM2_LABEL" \
    --camera camera3 "$CAM3_IMAGE_DIR" "$CAM3_INTRINSICS_DIR" "$CAM3_EXTRISICS_DIR" "$CAM3_LABEL" \
    --camera camera4 "$CAM4_IMAGE_DIR" "$CAM4_INTRINSICS_DIR" "$CAM4_EXTRISICS_DIR" "$CAM4_LABEL" \
    --camera camera5 "$CAM5_IMAGE_DIR" "$CAM5_INTRINSICS_DIR" "$CAM5_EXTRISICS_DIR" "$CAM5_LABEL" \
    --pcd_dir "$PCD_DIR" \
    --output_json_dir ./output_multicam_annotations
