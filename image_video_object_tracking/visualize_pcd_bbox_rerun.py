...
def visualize_sequence(pcd_dir, json_dir, fps=2):
    ...
    pcd_files = sorted([f for f in pcd_dir.glob("*.pcd")],
                       key=lambda p: p.stem)
...
def main():
    ...
    parser.add_argument("--pcd_dir", required=True, help="Path to directory of PCD files")
    parser.add_argument("--json_dir", required=True, help="Path to directory of bounding box JSONs")
    parser.add_argument("--fps", type=float, default=2.0, help="Playback FPS (default=2 FPS)")
